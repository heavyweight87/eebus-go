...
func (e *OHPCF) PowerConsumptionFlexibilitySettings(
	...
	if data.Alternatives[0].PowerSequence[0].State.State != nil {
		isAvaialable = (*data.Alternatives[0].PowerSequence[0].State.State == model.PowerSequenceStateTypeInactive)
	}
	...
	// } else {
	// 	earliestStartTime, err = data.Alternatives[0].PowerSequence[0].ScheduleConstraints.EarliestStartTime.GetTime()
	// 	if err != nil {
...
// StartTime retrieves the start time of the power consumption for the given entity.
func (e *OHPCF) StartTime(entity spineapi.EntityRemoteInterface) (*time.Time, error) {
	...
	util.DeepCopy[*model.SmartEnergyManagementPsDataType](data, &smartEnergyManagementPsData)
	sequance := &smartEnergyManagementPsData.Alternatives[0].PowerSequence[0]

	if sequance.Schedule == nil || sequance.Schedule.StartTime == nil {
		return nil, api.ErrDataNotAvailable
	}

	startTime, err := sequance.Schedule.StartTime.GetTime()
	if err != nil {
		return nil, err
	}
...
func (e *OHPCF) WriteStartTime(entity spineapi.EntityRemoteInterface, startTime time.Time) error {
	...
	sequance := &smartEnergyManagementPsData.Alternatives[0].PowerSequence[0]
	sequance.Schedule = &model.PowerSequenceScheduleDataType{
...
func (e *OHPCF) writeState(entity spineapi.EntityRemoteInterface, state model.PowerSequenceStateType) error {
	...
	util.DeepCopy(data, &smartEnergyManagementPsData)
	sequance := &smartEnergyManagementPsData.Alternatives[0].PowerSequence[0]
	sequance.State.State = util.Ptr(state)
